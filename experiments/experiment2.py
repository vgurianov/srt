...
class OriginalToolkit(ri.DataProcessing):
    ...
    def anl_solution(self):
        ...
        qe = (1/self.nu_m)*(self.nu_t*self.nu_t/self.nu_x) * 1.0/float(self.size_tick*self.size_tick)
        print "qE=", qe
        c = self.light_vel
...
class OriginalPrint(print_results.TablePrint):
    ...
    def sol_print_simple(self):
        ...
        for i in self.dt.obs.obt_g:
            print "Tw=",i,"t =",self.dt.t[i], "x =",round(self.dt.x[i],2), "xa =",round(self.dt.xa_track[i],2), "xe =",round(self.dt.xNtrack[i],2), "p =",self.dt.momentum_t[i], "pa=",round(self.dt.pa[i],2), "v=",round(self.dt.vel_t[i],2), "va=",round(self.dt.va[i],2)

    def sol_print_prettytable(self):
        print
        print "Analytical (xa, pa, va) and numerical (xe) solution"
        pt = print_results.PrettyTable(["Tw","t", "x", "xa", "xe", "p", "pa", "v", "va"])
        for i in self.dt.obs.obt_g:
            pt.add_row([i, self.dt.t[i], round(self.dt.x[i],2), round(self.dt.xa_track[i],2),round(self.dt.xn_track[i],2),round(self.dt.momentum_t[i],2),round(self.dt.pa[i],2),round(self.dt.vel_t[i],2), round(self.dt.va[i],2) ])
        print pt
...
# Run section
observer = ri.Table()
xt= simple_iteraction(size_tick, count_tick, particle_velocity, observer)
print
