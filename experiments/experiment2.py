...
class originalPrint(printResult.TablePrint):
    ...
    def solPrintPrettyTable(self):
        ...
        for i in self.dt.obs.obtG:
            pt.add_row([i, self.dt.t[i], round(self.dt.x[i],2), round(self.dt.xAtrack[i],2),round(self.dt.xNtrack[i],2),round(self.dt.momentum_t[i],2),round(self.dt.pA[i],2),round(self.dt.vel_t[i],2), round(self.dt.vA[i],2) ])
        print pt
...
# Run section
observer = ri.Table()
xt= simpleIteraction(sizeTick, countTick, particle_velocity, observer)
print
