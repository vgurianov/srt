...
class Leaf:
    ...
    def __init__(self, v):
        """ Parameters: v is velocity particle """
                ...
                if not (self.contents is None):
                    if self.contents.is_active():
                        ...
                        if not self._err_flag:
                            print "Cell has act:","for tt=marker=", tt.t, "cell=", self.x, "tloc=", self.tmp.t
                            self.contents.run()
                            self.observer.fix_it(t_glob, tt.t, self.x, tt.t, self.contents.tick)
                            self._err_flag = True
                        else:
                            self.observer.rewrite_it(t_glob, tt.t, self.x, tt.t, self.contents.tick)
                            print ",and cell has act:","for tt=marker=", tt.t, "cell=", self.x, "tloc=", self.tmp.t
