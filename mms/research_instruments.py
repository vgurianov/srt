...
class Table:
    ...
    def rewrite_it(self,tg, tt, xx, loc_t, prt_t):
        """ Rewrite data """
    ...
    def data_control(self):
        ...
        for i in range(0,self.count_tick):
            if self.obt_g[i] <> self.local_t[i]:
                print "No sync:obtG[i] <> localT[i]: ",i, self. obt_g[i], self.local_t[i]
            if self.obt_g[i] <> self.particle_t[i]:
                print "No sync:obtG[i] <> particleT[i]: ",i, self. obt_g[i], self.particle_t[i] 
...
class DataProcessing:
    ...
    def xt_accurate(self):
        ...
        for i in range(1,len(self.obs.obt_g)):
            an = math.sqrt(float(self.obs.obt_g[i])**2+self.x[i]**2)
    ...
    def momentum_calculate(self):
        ...
        for i in range(1,len(self.obs.obt_g)):
            ...
            f = (1.0/self.nu_m)*(1.0/float(self.size_tick))*(float(self.obs.pulse_t[i]))
            dp = f*dt
    ...
    def vel_analytical(self, p):
        ...
        pp=p
        v_anl = pp/math.sqrt(self.mass**2 + (pp/self.light_vel)**2)
        return v_anl
    # momentum
    ...
    def energe_calculate(self):
        ...
        for i in range(1,len(self.obs.obt_g)):
            ddt = float(self.obs.obt[i]-self.obs.obt[i-1])
            dt = self.t[i]-self.t[i-1]
            f = (1.0/self.nu_m)*(self.nu_t/float(self.size_tick))*(float(self.obs.pulse_t[i])/(float(self.size_tick)))
