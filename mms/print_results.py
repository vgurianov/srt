...
class TablePrint:
    ...
    def xt_print_simple(self):
        ...
        for i in self.dt.obs.obtG:
            print "Tlab=",i,"x =",self.dt.x[i], "t=", self.dt.t[i], "Accurate t=",round(self.dt.t_acc[i],2), "err%=", round(self.dt.t_local_err[i],2)
    ...
    def xt_print_prettytable(self):
        ...
        print "Trajectory of particle and local time"
        pt = PrettyTable()
        pt = PrettyTable(["Tlab","x", "t", "ta", "err%"])
    ...
    def vel_print_simple(self):
        ...
        for i in self.dt.obs.obt_g:
            if self.dt.vel_anl[i] == 0.0:
                ...
                err = math.fabs(100.0*(self.dt.vel_t[i]-self.dt.vel_anl[i])/self.dt.vel_anl[i])
            print "Tlab=",i,"p =",round(self.dt.momentum_t[i],2), "v=", round(self.dt.vel_t[i],2), "Analytical v=",round(self.dt.vel_anl[i],2), "err%=", round(err,2)
    ...
    def eng_print_simple(self):
        ...
        for i in self.dt.obs.obt_g:
            ...
            print "Tlab=",i,"p =",self.dt.momentum_t[i], "E=", round(self.dt.eng_t[i],2), "Analytical E=",round(self.dt.eng_t_acc[i],2), "err%=", round(err,2)
    ...
    def fromp_print_prettytable(self):
        ...
        pt = PrettyTable()
        pt = PrettyTable(["Tlab","p", "v", "va", "v,err%", "E", "Ea", "E,err%"])
        for i in self.dt.obs.obt_g:
            if self.dt.vel_anl[i] == 0.0:
                err1 = 0.0
            else:
                err1 = math.fabs(100.0*(self.dt.vel_t[i]-self.dt.vel_anl[i])/self.dt.vel_anl[i])
            err2 = math.fabs(100.0*(self.dt.eng_t[i]-self.dt.eng_t_acc[i])/self.dt.eng_t_acc[i])
 
            pt.add_row([i, round(self.dt.momentum_t[i],2), round(self.dt.vel_t[i],2), round(self.dt.vel_anl[i],2), round(err1,2), round(self.dt.eng_t[i],2), round(self.dt.eng_t_acc[i],2), round(err2,2) ])   
        print pt
